...
// Read-only strict execution inspector for the final A8 demo. It reads only
...
const subject = 'Localle demo reservation — Test Customer A8';
...
function inspect(executionId) {
  ...
  const checks = [
    ...
    [request.customer_name === 'Test Customer A8' && request.phone_raw === '+30 690 000 0006', 'real Request values'],
    [request.phone_raw === '+30 690 000 0006' && request.phone_raw !== '#ERROR!', 'phone_raw payload not #ERROR'],
    [Boolean(String(renderer.body ?? renderer.email_body ?? renderer.text ?? renderer.output ?? renderer.response ?? '').trim()), 'reply renderer non-empty'],
...
for (let attempt = 0; attempt < 720; attempt += 1) {
  ...
  if (found) break;
  process.stdout.write(attempt % 12 === 0 ? 'Waiting for A8 Gmail execution…\n' : '.');
  cp.execFileSync('sleep', ['5']);
}

if (!found) {
  console.log('\nLOCALLE A8 — FIX / REVIEW STILL REQUIRED');
  process.exitCode = 1;
} else {
  console.log(`\nInspecting execution ${found.id}`);
  const strict = inspect(found.id);
  if (!strict) {
    console.log('LOCALLE A8 — FIX / REVIEW STILL REQUIRED');
    process.exitCode = 1;
  } else {
    console.log('VISUAL SHEET CHECK: Requests is open. Confirm phone_raw visibly reads +30 690 000 0006 (no apostrophe and no #ERROR!), then type VERIFIED.');
    process.stdin.setEncoding('utf8');
    process.stdin.once('data', (answer) => {
      if (answer.trim() === 'VERIFIED') console.log('LOCALLE A8 LIVE E2E — STRICT PASS');
      else { console.log('LOCALLE A8 — FIX / REVIEW STILL REQUIRED'); process.exitCode = 1; }
    });
  }
}
